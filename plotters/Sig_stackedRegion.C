...
void Sig_stackedRegion(TString region, Bool_t dolog, Bool_t HIP, Bool_t doctau, Bool_t useEOS, TString description)
{
  ...
  for(unsigned int j=0; j<variables.size(); ++j){
   ...
   for(unsigned int tt=0; tt<loopEnd; ++tt){
	...
	cout<<"here"<<endl;
     ...
     h_bkgtotal= (TH1F*)h_Sig_MS55ct100->Clone("bkgtotal");
	cout<<"here"<<endl;

     // Integrals
     //Float_t  int_DY10to50                 = h_Sig_MS55ct10010to50                        ->Integral(0,-1); 
...
// integrals of summed histograms
     ...
     h_Sig_MS55ct100        ->SetFillStyle(1001);
     h_Sig_MS55ct100        ->SetLineColor(kBlack); 
     h_Sig_MS55ct100        ->SetLineWidth(0);
     h_bkgtotal->SetFillColorAlpha(kYellow+1, 0.7);
     h_bkgtotal->SetFillStyle(1001);

     std::vector<TH1F *> v;
     if(useAlt){
       v.push_back(h_Sig_MS55ct100);
     }
     else {
       v.push_back(h_Sig_MS55ct100);
     }
     ...
     if(useAlt){
       leg->AddEntry(h_Sig_MS55ct100        , "ZH4b", "f"); 
	...
	char charlegendprint[80];
     TLegend *sigleg = new TLegend(0.54,0.6,0.88,0.88);
     if(drawSignal){
       sigleg->SetBorderSize(0);
       sigleg->SetFillColor(kWhite);
	string legendprint = "ZH4b_MS55_ct100";
	legendprint += uncbin;
	strcpy(charlegendprint,legendprint.c_str());	
       ...
       sigleg->AddEntry(h_Sig_MS55ct100_NoUnc   , "ZH4b_MS55_ct100 NoUnc  ", "l" ) ;
	if(uncbin=="_MESUp") sigleg->AddEntry(h_Sig_MS55ct100_MESDown   , "ZH4b_MS55_ct100 MESDown", "l" ) ;
	if(uncbin=="_EGSUp") sigleg->AddEntry(h_Sig_MS55ct100_EGSDown   , "ZH4b_MS55_ct100 EGSDown", "l" ) ;
	if(uncbin=="_LeptonSFUp") sigleg->AddEntry(h_Sig_MS55ct100_LeptonSFDown   , "ZH4b_MS55_ct100 LeptonSFDown", "l" ) ;
	if(uncbin=="_AMaxUp") sigleg->AddEntry(h_Sig_MS55ct100_AMaxDown   , "ZH4b_MS55_ct100 AMaxDown", "l" ) ;
	if(uncbin=="_IPSigUp") sigleg->AddEntry(h_Sig_MS55ct100_IPSigDown   , "ZH4b_MS55_ct100 IPSigDown", "l" ) ;
	if(uncbin=="_TAUp") sigleg->AddEntry(h_Sig_MS55ct100_TADown   , "ZH4b_MS55_ct100 TADown", "l" ) ;
	if(uncbin=="_TagVarsUp") sigleg->AddEntry(h_Sig_MS55ct100_TagVarsDown   , "ZH4b_MS55_ct100 TagVarsDown", "l" ) ;
     }
...
*/
 //    std::cout << drawSignal <<std::endl;
     // set max and draw
     double_t ymax;
     ymax = std::max(h_Sig_MS55ct100->GetMaximum(), h_bkgtotal->GetMaximum() );
     if(dolog){
      //bgstack->SetMaximum(50000*ymax); 
      //bgstack->SetMinimum(1.0e-6);
      bgstack->SetMaximum(1000); 
      bgstack->SetMinimum(1.0e-3);
     } 
     else {
	...
	if(uncbin == "_EGSUp"){
       h_Sig_MS55ct100_EGSDown->SetLineColor(kBlue);
	...
	if(uncbin == "_LeptonSFUp"){
       h_Sig_MS55ct100_LeptonSFDown->SetLineColor(kBlue);
	...
	if(uncbin == "_AMaxUp"){
       h_Sig_MS55ct100_AMaxDown->SetLineColor(kBlue);
       h_Sig_MS55ct100_AMaxDown->SetLineWidth(4);
       h_Sig_MS55ct100_AMaxDown->SetLineStyle(2);
       h_Sig_MS55ct100_AMaxDown->Draw("hist sames") ;
	}
	if(uncbin == "_IPSigUp"){
       h_Sig_MS55ct100_IPSigDown->SetLineColor(kBlue);
       h_Sig_MS55ct100_IPSigDown->SetLineWidth(4);
       h_Sig_MS55ct100_IPSigDown->SetLineStyle(2);
       h_Sig_MS55ct100_IPSigDown->Draw("hist sames") ;
	}
	if(uncbin == "_TAUp"){
       h_Sig_MS55ct100_TADown->SetLineColor(kBlue);
       h_Sig_MS55ct100_TADown->SetLineWidth(4);
       h_Sig_MS55ct100_TADown->SetLineStyle(2);
       h_Sig_MS55ct100_TADown->Draw("hist sames") ;
	}
	if(uncbin == "_TagVarsUp"){
       h_Sig_MS55ct100_TagVarsDown->SetLineColor(kBlue);
       h_Sig_MS55ct100_TagVarsDown->SetLineWidth(4);
       h_Sig_MS55ct100_TagVarsDown->SetLineStyle(2);
       h_Sig_MS55ct100_TagVarsDown->Draw("hist sames") ;
	}
       sigleg->Draw();
 }
	/*	
	...
	if(uncbin == "_MESUp"){
        h_ratio2 = (TH1F*)h_Sig_MS55ct100_MESDown->Clone("ratio");
	h_ratio2->Divide(h_Sig_MS55ct100_NoUnc);
	}
	if(uncbin == "_EGSUp"){
        h_ratio2 = (TH1F*)h_Sig_MS55ct100_EGSDown->Clone("ratio");
  	h_ratio2->Divide(h_Sig_MS55ct100_NoUnc);
	}
	if(uncbin == "_LeptonSFUp"){
        h_ratio2 = (TH1F*)h_Sig_MS55ct100_LeptonSFDown->Clone("ratio");
	h_ratio2->Divide(h_Sig_MS55ct100_NoUnc);
	}
	if(uncbin == "_AMaxUp"){
        h_ratio2 = (TH1F*)h_Sig_MS55ct100_AMaxDown->Clone("ratio");
	h_ratio2->Divide(h_Sig_MS55ct100_NoUnc);
	}
	if(uncbin == "_IPSigUp"){
        h_ratio2 = (TH1F*)h_Sig_MS55ct100_IPSigDown->Clone("ratio");
	h_ratio2->Divide(h_Sig_MS55ct100_NoUnc);
	}
	if(uncbin == "_TAUp"){
        h_ratio2 = (TH1F*)h_Sig_MS55ct100_TADown->Clone("ratio");
	h_ratio2->Divide(h_Sig_MS55ct100_NoUnc);
	}
	if(uncbin == "_TagVarsUp"){
        h_ratio2 = (TH1F*)h_Sig_MS55ct100_TagVarsDown->Clone("ratio");
	h_ratio2->Divide(h_Sig_MS55ct100_NoUnc);
	}
       h_ratio->SetTitle(" ");
	...
	if(uncbin == "_AMaxUp"|| uncbin == "_IPSigUp" || uncbin == "_TAUp" || uncbin == "_TagVarsUp"){h_ratio->GetYaxis()->SetRangeUser(0.0,2.0);}
     ...
     // save canvas
     canvas->SaveAs(outname+description+"_Sig.png");
     canvas->SaveAs(outname+description+"_Sig.pdf");
  
// save histograms into single root file
     TFile *outfile = TFile::Open(outname+"_Sig.root","RECREATE");

...
Float_t cal_0u(Float_t orig, Float_t unc){
  Float_t delta = unc-orig;
  Float_t value;
  value = 1-delta/(1-orig);

  return value;
}

Float_t cal_1u(Float_t orig, Float_t unc){
  Float_t delta = unc-orig;
  Float_t value;
  value = 1+(delta-2*delta*orig-delta*delta)/(orig*(1-orig));

  return value;
}

Float_t cal_2u(Float_t orig, Float_t unc){
  Float_t delta = unc-orig;
  Float_t value;
  value = 1+(2*delta*orig+delta*delta-3*delta*orig*orig-3*delta*delta*orig-pow(delta,3))/(orig*orig*(1-orig));

  return value;
}

Float_t cal_0d(Float_t orig, Float_t unc){
  Float_t delta = unc-orig;
  Float_t value;
  value = 1+delta/(1-orig);
...
Float_t cal_1d(Float_t orig, Float_t unc){
  Float_t delta = unc-orig;
  Float_t value;
  value = 1+(-1*delta+2*delta*orig-delta*delta)/(orig*(1-orig));

  return value;
}

Float_t cal_2d(Float_t orig, Float_t unc){
  Float_t delta = unc-orig;
  Float_t value;
  value = 1+(-2*delta*orig+delta*delta+3*delta*orig*orig-3*delta*delta*orig+pow(delta,3))/(orig*orig*(1-orig));
...
void Sig_calcout(TString region, Bool_t dolog, Bool_t HIP, Bool_t doctau, Bool_t useEOS, TString description)
{
...
cout<<"where"<<endl;
Float_t unc_1_AMUp  	        = cal_0u(n_AM_Pass,n_AM_UpPass);
Float_t unc_1_AMDown  	        = cal_0d(n_AM_Pass,n_AM_DownPass);
Float_t unc_1_IPSigUp  	        = cal_0u(n_IPSig_Pass,n_IPSig_UpPass);
Float_t unc_1_IPSigDown  	= cal_0d(n_IPSig_Pass,n_IPSig_DownPass);
Float_t unc_1_TAUp  	        = cal_0u(n_TA_Pass,n_TA_UpPass);
Float_t unc_1_TADown  	        = cal_0d(n_TA_Pass,n_TA_DownPass);
Float_t unc_2_AMUp  	        = cal_1u(n_AM_Pass,n_AM_UpPass);
Float_t unc_2_AMDown  	        = cal_1d(n_AM_Pass,n_AM_DownPass);
Float_t unc_2_IPSigUp  	        = cal_1u(n_IPSig_Pass,n_IPSig_UpPass);
Float_t unc_2_IPSigDown  	= cal_1d(n_IPSig_Pass,n_IPSig_DownPass);
Float_t unc_2_TAUp  	        = cal_1u(n_TA_Pass,n_TA_UpPass);
Float_t unc_2_TADown  	        = cal_1d(n_TA_Pass,n_TA_DownPass);
Float_t unc_3_AMUp  	        = cal_2u(n_AM_Pass,n_AM_UpPass);
Float_t unc_3_AMDown  	        = cal_2d(n_AM_Pass,n_AM_DownPass);
Float_t unc_3_IPSigUp  	        = cal_2u(n_IPSig_Pass,n_IPSig_UpPass);
Float_t unc_3_IPSigDown  	= cal_2d(n_IPSig_Pass,n_IPSig_DownPass);
Float_t unc_3_TAUp  	        = cal_2u(n_TA_Pass,n_TA_UpPass);
Float_t unc_3_TADown  	        = cal_2d(n_TA_Pass,n_TA_DownPass);
...
Float_t alsounc_wWeight_3_TADown  	        = cal_2d(n_TA_alsoPass_wWeight,n_TA_DownalsoPass_wWeight);
     ...
     if(domaketable){
      FILE * outfulltable;
      outfulltable = fopen (fulllogname+MSsample+"new.tex","w");
